...
export default function Agents() {
  ...
  const { agentLogs } = useLiveFeed();
  
  const agents = (data ?? []).map(a => {
    const latest = agentLogs?.[a.id]?.[0];
    return {
      ...a,
      status: latest?.status || a.status,
      heartbeat: latest?.timestamp || a.heartbeat
    };
  });
              ...
              <div key={agent.id} className="card" style={{
                ...
                <div style={{ display:"flex", alignItems:"center", gap:5 }}>
                  ...
                  <span style={{ fontSize:10, color:"var(--text-secondary)" }}>
                    Last heartbeat: {timeAgo(agent.heartbeat || new Date().toISOString())}
                  </span>
                </div>

                {/* Terminal Visualization */}
                <div style={{
                  background:"#0A0A0A", border:"1px solid #222", borderRadius:"6px", padding:"10px", marginTop:"16px", minHeight:"90px"
                }}>
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"8px" }}>
                    <span style={{ fontSize:9, color:"#10B981", fontFamily:"monospace", letterSpacing:"0.05em" }}>LIVE TERMINAL</span>
                    <span style={{ display:"inline-block", width:6, height:6, borderRadius:"50%", background:"#10B981", boxShadow:"0 0 8px #10B981", animation:"pulse 2s infinite" }} />
                  </div>
                  <div style={{ display:"flex", flexDirection:"column", gap:"4px" }}>
                    {(agentLogs?.[agent.id] || []).map((log, idx) => (
                      <div key={idx} style={{ fontSize:10, fontFamily:"monospace", color: idx === 0 ? "#fff" : "#555" }}>
                        <span style={{ color: idx === 0 ? "#10B981" : "#444", marginRight:"6px" }}>&gt;</span>
                        {log.msg}
                      </div>
                    ))}
                    {!(agentLogs?.[agent.id]?.length) && (
                      <div style={{ fontSize:10, fontFamily:"monospace", color:"#555" }}>
                        <span style={{ color:"#444", marginRight:"6px" }}>&gt;</span>
