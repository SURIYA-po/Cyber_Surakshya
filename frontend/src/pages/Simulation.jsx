...
import { simulateAttack, uploadZeek } from "../api/simulationApi";
  ...
  async function handleUpload(e) {
    ...
    try {
      ...
      if (data.attacks_found === 0) {
        throw new Error("No attacks found in this log file (all records were BENIGN).");
      }
      
      // Show the first detected attack on the dashboard
      setResult({
        alert: data.alerts[0],
        analysis: data.analyses[0],
        threat_score: { score: 98, risk: "HIGH", priority: data.alerts[0]?.severity },
        response: { action: "Block Source IP", status: "Executed" },
        blocked_ip: data.blocked_ips[0]
      });
        ...
        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:"1.5rem", flexWrap:"wrap" }}>
          ...
          <div style={{ display:"flex", gap:"0.75rem", flexWrap:"wrap" }}>
            ...
            <button 
              ...
              onClick={() => document.getElementById("zeek-upload").click()} 
              disabled={running} 
              style={{ flexShrink:0 }}
            >
              Upload conn.log
